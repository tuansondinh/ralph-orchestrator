# Funding Ralph Orchestrator

If Ralph Orchestrator helps your team ship faster, consider sponsoring ongoing development.

## Sponsor

- GitHub Sponsors: https://github.com/sponsors/mikeyobrien

## What sponsorship supports

- Core orchestration and reliability improvements
- Adapter and backend maintenance
- Documentation, examples, and onboarding guides
- CI/test infrastructure and release operations

## Other ways to help

- Star the repo and share it with your team
- Open issues with reproducible bug reports
- Contribute code, docs, tests, or fixtures

Thanks for supporting open source ❤️
